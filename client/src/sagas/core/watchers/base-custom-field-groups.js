...
export default function* baseCustomFieldGroupsWatchers() {
  yield all([
    takeEvery(
      ...
      ({ payload: { data } }) =>
        services.createBaseCustomFieldGroupInCurrentProject(data)
    ),
    takeEvery(
    ...
    takeEvery(
      EntryActionTypes.BASE_CUSTOM_FIELD_GROUP_UPDATE,
      ({ payload: { id, data } }) =>
    ...
    takeEvery(
      EntryActionTypes.BASE_CUSTOM_FIELD_GROUP_DELETE,
      ({ payload: { id } }) => services.deleteBaseCustomFieldGroup(id)
    ),
    takeEvery(
