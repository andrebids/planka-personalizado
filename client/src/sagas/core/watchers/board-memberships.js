...
export default function* boardMembershipsWatchers() {
  yield all([
    takeEvery(
      EntryActionTypes.MEMBERSHIP_IN_CURRENT_BOARD_CREATE,
      ({ payload: { data } }) => services.createMembershipInCurrentBoard(data)
    ),
    takeEvery(
    ...
    takeEvery(
      EntryActionTypes.BOARD_MEMBERSHIP_UPDATE,
      ({ payload: { id, data } }) => services.updateBoardMembership(id, data)
    ),
    takeEvery(
      EntryActionTypes.BOARD_MEMBERSHIP_UPDATE_HANDLE,
    ...
    takeEvery(
      EntryActionTypes.BOARD_MEMBERSHIP_DELETE_HANDLE,
      ({ payload: { boardMembership } }) =>
