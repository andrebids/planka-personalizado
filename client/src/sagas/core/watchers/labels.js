...
export default function* labelsWatchers() {
  yield all([
    takeEvery(
      ...
      ({ payload: { data } }) => services.createLabelInCurrentBoard(data)
    ),
    takeEvery(
      EntryActionTypes.LABEL_FROM_CARD_CREATE,
      ({ payload: { cardId, data } }) =>
    ...
    takeEvery(
      ...
      ({ payload: { id, cardId } }) => services.addLabelToCard(id, cardId)
    ),
    takeEvery(
      EntryActionTypes.LABEL_TO_CURRENT_CARD_ADD,
      ({ payload: { id } }) => services.addLabelToCurrentCard(id)
    ),
    takeEvery(
      EntryActionTypes.LABEL_TO_CARD_ADD_HANDLE,
      ({ payload: { cardLabel } }) => services.handleLabelToCardAdd(cardLabel)
    ),
    takeEvery(
      EntryActionTypes.LABEL_FROM_CARD_REMOVE,
      ({ payload: { id, cardId } }) => services.removeLabelFromCard(id, cardId)
    ),
    takeEvery(
      EntryActionTypes.LABEL_FROM_CURRENT_CARD_REMOVE,
      ({ payload: { id } }) => services.removeLabelFromCurrentCard(id)
    ),
    takeEvery(
      EntryActionTypes.LABEL_FROM_CARD_REMOVE_HANDLE,
      ({ payload: { cardLabel } }) =>
        services.handleLabelFromCardRemove(cardLabel)
    ),
    takeEvery(
      EntryActionTypes.LABEL_TO_FILTER_IN_CURRENT_BOARD_ADD,
      ({ payload: { id } }) => services.addLabelToFilterInCurrentBoard(id)
    ),
    takeEvery(
      EntryActionTypes.LABEL_FROM_FILTER_IN_CURRENT_BOARD_REMOVE,
      ({ payload: { id } }) => services.removeLabelFromFilterInCurrentBoard(id)
    ),
  ]);
}
