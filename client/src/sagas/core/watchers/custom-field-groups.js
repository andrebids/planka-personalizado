...
export default function* customFieldGroupsWatchers() {
  yield all([
    takeEvery(
      ...
      ({ payload: { data } }) =>
        services.createCustomFieldGroupInCurrentBoard(data)
    ),
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_GROUP_IN_CURRENT_CARD_CREATE,
      ({ payload: { data } }) =>
    ...
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_GROUP_UPDATE,
      ({ payload: { id, data } }) => services.updateCustomFieldGroup(id, data)
    ),
    takeEvery(
    ...
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_GROUP_MOVE,
      ({ payload: { id, index } }) => services.moveCustomFieldGroup(id, index)
    ),
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_GROUP_DELETE,
      ({ payload: { id } }) => services.deleteCustomFieldGroup(id)
    ),
    takeEvery(
