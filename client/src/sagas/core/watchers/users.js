...
export default function* usersWatchers() {
  yield all([
    takeEvery(EntryActionTypes.USER_CREATE, ({ payload: { data } }) =>
    ...
    takeEvery(
      ...
      ({ payload: { data } }) => services.updateCurrentUserEmail(data)
    ),
    takeEvery(
      EntryActionTypes.USER_EMAIL_UPDATE_ERROR_CLEAR,
      ({ payload: { id } }) => services.clearUserEmailUpdateError(id)
    ),
    takeEvery(EntryActionTypes.CURRENT_USER_EMAIL_UPDATE_ERROR_CLEAR, () =>
      services.clearCurrentUserEmailUpdateError()
    ),
    takeEvery(
      EntryActionTypes.USER_PASSWORD_UPDATE,
      ({ payload: { id, data } }) => services.updateUserPassword(id, data)
    ),
    takeEvery(
      EntryActionTypes.CURRENT_USER_PASSWORD_UPDATE,
      ({ payload: { data } }) => services.updateCurrentUserPassword(data)
    ),
    takeEvery(
      EntryActionTypes.USER_PASSWORD_UPDATE_ERROR_CLEAR,
      ({ payload: { id } }) => services.clearUserPasswordUpdateError(id)
    ),
    takeEvery(EntryActionTypes.CURRENT_USER_PASSWORD_UPDATE_ERROR_CLEAR, () =>
      services.clearCurrentUserPasswordUpdateError()
    ),
    takeEvery(
      EntryActionTypes.USER_USERNAME_UPDATE,
      ({ payload: { id, data } }) => services.updateUserUsername(id, data)
    ),
    takeEvery(
      EntryActionTypes.CURRENT_USER_USERNAME_UPDATE,
      ({ payload: { data } }) => services.updateCurrentUserUsername(data)
    ),
    takeEvery(
      EntryActionTypes.USER_USERNAME_UPDATE_ERROR_CLEAR,
      ({ payload: { id } }) => services.clearUserUsernameUpdateError(id)
    ),
    takeEvery(EntryActionTypes.CURRENT_USER_USERNAME_UPDATE_ERROR_CLEAR, () =>
      services.clearCurrentUserUsernameUpdateError()
    ),
    takeEvery(
      EntryActionTypes.CURRENT_USER_AVATAR_UPDATE,
      ({ payload: { data } }) => services.updateCurrentUserAvatar(data)
    ),
    takeEvery(EntryActionTypes.USER_DELETE, ({ payload: { id } }) =>
    ...
    takeEvery(
      ...
      ({ payload: { id, replace } }) =>
        services.addUserToFilterInCurrentBoard(id, replace)
    ),
    takeEvery(
      EntryActionTypes.USER_FROM_FILTER_IN_CURRENT_BOARD_REMOVE,
      ({ payload: { id } }) => services.removeUserFromFilterInCurrentBoard(id)
    ),
  ]);
}
