...
export default function* projectsWatchers() {
  yield all([
    ...
    takeEvery(
      EntryActionTypes.PROJECTS_ORDER_UPDATE,
    ...
    takeEvery(
      EntryActionTypes.PROJECT_CREATE_HANDLE,
      ({ payload: { project } }) => services.handleProjectCreate(project)
    ),
    ...
    takeEvery(
      ...
      ({ payload: { data } }) => services.updateCurrentProject(data)
    ),
    takeEvery(
      EntryActionTypes.PROJECT_UPDATE_HANDLE,
      ({ payload: { project } }) => services.handleProjectUpdate(project)
    ),
    takeEvery(EntryActionTypes.CURRENT_PROJECT_DELETE, () =>
      services.deleteCurrentProject()
    ),
    takeEvery(
      EntryActionTypes.PROJECT_DELETE_HANDLE,
      ({ payload: { project } }) => services.handleProjectDelete(project)
    ),
  ]);
}
