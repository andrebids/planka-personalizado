...
export default function* customFieldsWatchers() {
  yield all([
    ...
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_CREATE_HANDLE,
      ({ payload: { customField } }) =>
        services.handleCustomFieldCreate(customField)
    ),
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_UPDATE,
      ({ payload: { id, data } }) => services.updateCustomField(id, data)
    ),
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_UPDATE_HANDLE,
      ({ payload: { customField } }) =>
        services.handleCustomFieldUpdate(customField)
    ),
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_MOVE,
      ({ payload: { id, index } }) => services.moveCustomField(id, index)
    ),
    takeEvery(EntryActionTypes.CUSTOM_FIELD_DELETE, ({ payload: { id } }) =>
      services.deleteCustomField(id)
    ),
    takeEvery(
      EntryActionTypes.CUSTOM_FIELD_DELETE_HANDLE,
      ({ payload: { customField } }) =>
