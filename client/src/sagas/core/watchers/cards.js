...
export default function* cardsWatchers() {
  yield all([
    ...
    takeEvery(
      EntryActionTypes.CARDS_UPDATE_HANDLE,
      ({ payload: { cards, activities } }) =>
    ...
    takeEvery(
      ...
      ({ payload: { listId, index, autoClose } }) =>
    ...
    takeEvery(EntryActionTypes.CURRENT_CARD_TO_TRASH_MOVE, () =>
      services.moveCurrentCardToTrash()
    ),
    takeEvery(
    ...
    takeEvery(
      EntryActionTypes.CURRENT_CARD_DUPLICATE,
      ({ payload: { data } }) => services.duplicateCurrentCard(data)
    ),
    takeEvery(
      EntryActionTypes.TO_ADJACENT_CARD_GO,
      ({ payload: { direction } }) => services.goToAdjacentCard(direction)
    ),
    takeEvery(EntryActionTypes.CARD_DELETE, ({ payload: { id } }) =>
      services.deleteCard(id)
    ),
