...
export default function* boardsWatchers() {
  yield all([
    takeEvery(
      ...
      ({ payload: { data } }) => services.createBoardInCurrentProject(data)
    ),
    takeEvery(
    ...
    takeEvery(EntryActionTypes.BOARD_FETCH, ({ payload: { id } }) =>
      services.fetchBoard(id)
    ),
    ...
    takeEvery(
      EntryActionTypes.CONTEXT_IN_CURRENT_BOARD_UPDATE,
      ({ payload: { value } }) => services.updateContextInCurrentBoard(value)
    ),
    takeEvery(
      EntryActionTypes.VIEW_IN_CURRENT_BOARD_UPDATE,
      ({ payload: { value } }) => services.updateViewInCurrentBoard(value)
    ),
    takeEvery(
