...
export default function* notificationServicesWatchers() {
  yield all([
    takeEvery(
      ...
      ({ payload: { data } }) =>
        services.createNotificationServiceInCurrentUser(data)
    ),
    takeEvery(
      EntryActionTypes.NOTIFICATION_SERVICE_IN_BOARD_CREATE,
      ({ payload: { boardId, data } }) =>
        services.createNotificationServiceInBoard(boardId, data)
    ),
    takeEvery(
    ...
    takeEvery(
      EntryActionTypes.NOTIFICATION_SERVICE_UPDATE,
      ({ payload: { id, data } }) =>
    ...
    takeEvery(
      EntryActionTypes.NOTIFICATION_SERVICE_TEST,
      ({ payload: { id } }) => services.testNotificationService(id)
    ),
    takeEvery(
      EntryActionTypes.NOTIFICATION_SERVICE_DELETE,
      ({ payload: { id } }) => services.deleteNotificationService(id)
    ),
    takeEvery(
